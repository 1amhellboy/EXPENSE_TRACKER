...
def update_budget_summary():
    try:
        ...
        remaining_budget = budget - float(total_label.cget("text").split(":")[1].strip())
        budget_summary_label.config(text=f"Remaining Budget: {remaining_budget:.2f}")
    except ValueError:
        budget_summary_label.config(text="Invalid Budget", fg="red")
...
category_label = tk.Label(root, text="Category/Item:")
...
budget_summary_label = tk.Label(root, text="Remaining Budget: N/A", fg="black")
budget_summary_label.grid(row=7, column=0, columnspan=2, padx=5, pady=5)
...
delete_button.grid(row=9, column=1, padx=5, pady=10)
